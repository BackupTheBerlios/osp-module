...
void getNextHop(struct sip_msg* msg, char* next_hope)
{
		...
		if (hf->type == HDR_ROUTE) {
			...
			if (parse_uri(route->nameaddr.uri.s,route->nameaddr.uri.len,&puri) == 0) {
				...
				if (check_self(&puri.host, puri.port_no ? puri.port_no : SIP_PORT, PROTO_NONE) != 1) {
					...
					strncpy(next_hope,puri.host.s,puri.host.len);
					next_hope[puri.host.len] = '\0';
					found = 1;
					break;
				} else {
	...
	if (!found) {
		...
		strncpy(next_hope,msg->parsed_uri.host.s,msg->parsed_uri.host.len);
		next_hope[msg->parsed_uri.host.len] = '\0';
		found =1;
	}
}
